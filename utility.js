...
const updateCatFact = async (fact) => {
    const catFactsArray = await getAllFacts();
    let filteredCatFactsArray = catFactsArray.filter((f) => f.id !== parseInt(fact.id, 10));
    filteredCatFactsArray1 = [...filteredCatFactsArray, fact];
    const json =  JSON.stringify(filteredCatFactsArray1, null, 2);
    writeToFile(json); 
    return(filteredCatFactsArray1)
}
